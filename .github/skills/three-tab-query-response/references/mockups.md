...
**⚠️ Design Philosophy Note:**

WrenAI's team likely chose NOT to cache query results for a strategic reason:

**Their Design Philosophy:**
- If a user finds a useful chart/query, they should **pin it to the dashboard**
- The dashboard becomes the "saved results" mechanism
- Pinned charts have explicit refresh controls
- This avoids stale data confusion (user doesn't know if thread data is fresh)
- Clear separation: Threads = exploration, Dashboard = saved insights

**Trade-offs:**
- ✅ **Pro:** Users explicitly choose what to save (reduces clutter)
- ✅ **Pro:** Dashboard provides clear data freshness indicators
- ✅ **Pro:** No cache invalidation complexity
- ❌ **Con:** Re-opening threads always refetches (slower UX)
- ❌ **Con:** Exploratory analysis in threads feels slower
- ❌ **Con:** Wastes database resources on repeated queries

**Recommendation for Your App:**

Consider a **hybrid approach**:
1. **Short TTL cache (5-15 minutes)** for thread responses
   - Covers immediate back-and-forth exploration
   - Expires quickly to avoid stale data
2. **Dashboard as long-term storage**
   - Users pin important charts (no expiry)
   - Explicit refresh controls
3. **Clear indicators**
   - Show "Last refreshed: 2 minutes ago" in threads
   - Show "Last updated: 10 minutes ago" on dashboard

This balances exploration speed with data freshness clarity.

**Benefits:**
- Instant load for recent threads (within TTL window)
- Reduces database load for exploratory workflows
- Users still pin important insights to dashboard
